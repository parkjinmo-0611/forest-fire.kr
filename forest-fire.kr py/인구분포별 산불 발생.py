...
sns.regplot(data=df_merged, x='인구수', y='산불건수')

for i in range(df_merged.shape[0]):
    plt.text(df_merged.인구수[i], df_merged.산불건수[i], df_merged.지역[i], fontsize=11)

plt.title('시도별 인구수와 산불 발생 건수의 관계', fontsize=15)
plt.xlabel('인구수 (명)')
plt.ylabel('산불 발생 건수 (10년 평균)')
plt.show()
